...
#define TASK1_INIT_FLAGS		((1 << 3) | (1 << 5))
...
void task1_entry (void * arg) {
	...
	// 不满足，部分置1，超时
	os_err_t err;
	os_flags_t flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL, (1 << 3) | (1 << 2), &err);
	if (err < 0) {
	...
	// 部分置1，不满足，不超时
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL, (1 << 3), &err);
	if (err < 0) {
		os_dbg("eflags[1]: wait tmo, err: -------------\n");
	}
	os_dbg("eflags[1]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));
	
	// 多了位置1，不满足，超时
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL, TASK1_INIT_FLAGS | (1 << 8), &err);
	if (err < 0) {
	...
	// 全部一样，不超时
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL, TASK1_INIT_FLAGS, &err);
	if (err < 0) {
		os_dbg("eflags[3]: wait tmo, err: -------------\n");
	}
	os_dbg("eflags[3]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));

	/// 下面测试消耗
	// 不满足，且不更改eflags的值
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL | OS_EOPT_EXIT_CLEAR, TASK1_INIT_FLAGS | (1 << 8), &err);
	if (err < 0) {
		os_dbg("eflags[4]: wait tmo\n");
	}
	os_dbg("eflags[4]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));

	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL | OS_EOPT_EXIT_CLEAR, (1 << 3), &err);
	if (err < 0) {
