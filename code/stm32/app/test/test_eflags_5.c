...
#define TASK1_INIT_FLAGS		0xFF		// 1 << 0 和 1<< 2均为0
...
void task1_entry (void * arg) {
	...
	// 不满足，挂起
	os_err_t err;
	os_flags_t flags = os_eflags_wait(task1_eflags, 0, OS_EOPT_CLEAR_ALL, (1 << 7) | (1 << 1), &err);
	if (err < 0) {
...
void task2_entry (void * arg) {
	...
	os_eflags_release(task1_eflags, OS_EOPT_CLEAR, (1 << 7));
	os_dbg("eflags[1]: release %x, wait task: %d\n", os_eflags_flags(task1_eflags, OS_NULL), os_eflags_tasks(task1_eflags));

	os_eflags_release(task1_eflags, OS_EOPT_CLEAR, (1 << 1));
