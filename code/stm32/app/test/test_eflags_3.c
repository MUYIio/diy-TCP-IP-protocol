...
void task1_entry (void * arg) {
	...
	// 满足，均为0
	os_err_t err;
	os_flags_t flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL, (1 << 2) | (1 << 0), &err);
	if (err < 0) {
	...
	// 不满足，只有部分为1
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_SET_ALL, 1 << 2, &err);
	if (err < 0) {
	...
	// 不满足，等待的位全为1
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL, 1 << 4, &err);
	if (err < 0) {
	...
	/// 下面测试消耗
	// 不满足，且不更改eflags的值
	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL | OS_EOPT_EXIT_CLEAR, (1 << 4), &err);
	if (err < 0) {
		os_dbg("eflags[4]: wait tmo\n");
	}
	os_dbg("eflags[4]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));

	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL | OS_EOPT_EXIT_CLEAR, (1 << 2), &err);
	if (err < 0) {
		os_dbg("eflags[5]: wait tmo, err: -------------\n");
	}
	os_dbg("eflags[5]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));

	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL | OS_EOPT_EXIT_CLEAR, (1 << 2) | (1 << 0), &err);
	if (err < 0) {
		os_dbg("eflags[6]: wait tmo\n");
	}
	os_dbg("eflags[6]: %x\n", os_eflags_flags(task1_eflags, OS_NULL));

	flags = os_eflags_wait(task1_eflags, 1000, OS_EOPT_CLEAR_ALL | OS_EOPT_EXIT_CLEAR, (1 << 0), &err);
	if (err < 0) {
