...
ipaddr_t ipadd_get_net (const ipaddr_t * ipaddr, const ipaddr_t * netmask) {
    ipaddr_t netid;
...
// 192.168.74.1   -- .74.255  255.255.255.255
int ipaddr_is_match(const ipaddr_t * dest, const ipaddr_t * src, const ipaddr_t * netmask) {
    ipaddr_t dest_netid = ipadd_get_net(dest, netmask);
    ipaddr_t src_netid = ipadd_get_net(dest, netmask);
